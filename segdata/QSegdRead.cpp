...
// for Sercel 2008.6.27 for m_iTraceGapBytes
   ...
   m_iDataBytes = ltr * 1000 / m_infoGHD.iSi * m_fFormatBytes ;//+ m_iTraceGapBytes;//201610 deleted

   bytes = m_iDataHDBytes + m_iDataBytes + m_iTraceGapBytes;
...
//2
         qDebug("version : = %f", m_infoGHD.version);
...
//	convert
   if (m_iSI != 0) len = m_iLTR * 1000 / m_iSI;
...
//first header:---------20bytes-----------------------------------
   ...
   // file: 1-2 ; bcd
   if (m_pcBuf[0] != 0xff) iFile = BCD2Int(m_pcBuf + 1 - 1, 4, true); //1-2
   else iFile = -1;
   // scan type: 3;bcd
   iScan = BCD2Int(m_pcBuf + 3 - 1, 2, true);     //3
                                                  // channel sets: 4;bcd
   if (m_pcBuf[0] != 0xff) iCHsets = BCD2Int(m_pcBuf + 3, 2, true);   //4
   else iCHsets = -1;
   ...
   if (iCHsets == -1) iCHsets = getShort(m_pcBuf + 16 - 1); //16-17

   if (iFile == -1) iFile = getInt3(m_pcBuf + 18 - 1); //18-20
